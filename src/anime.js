...
let animationDocument = document;
...
function hasParentPrototypOf(element, prototypeName) {
    const parent = Object.getPrototypeOf(element);
    if (parent === null) {
        return false;
    } else if (parent.constructor.name === prototypeName) {
        return true;
    } else {
        return hasParentPrototypOf(parent, prototypeName);
    }
}
...
const is = {
    ...
    svg: a => hasParentPrototypOf(a, "SVGElement"),
    inp: a => hasParentPrototypOf(a, "HTMLInputElement"),
...
function selectString(str) {
    try {
        let nodes = animationDocument.querySelectorAll(str);
        return nodes;
    } catch (e) {
        return;
    }
...
function toArray(o) {
    ...
    if (o.constructor.name === "NodeList" || o.constructor.name === "HTMLCollection") return [].slice.call(o);
    return [o];
}
...
function convertPxToUnit(el, value, unit) {
    ...
    const baseline = 100;
    const tempEl = animationDocument.createElement(el.tagName);
    const parentEl = (el.parentNode && (el.parentNode !== animationDocument)) ? el.parentNode : animationDocument.body;
...
const engine = (() => {
    ...
    if (typeof document !== 'undefined') {
        animationDocument.addEventListener('visibilitychange', handleVisibilityChange);
    }
...
function isDocumentHidden() {
    return !!animationDocument && animationDocument.hidden;
}
...
function setAnimationDocument(document) {
...
anime.setAnimationDocument = setAnimationDocument;
